...
class MQTT extends events.EventEmitter {
    ...
    async connect() {
        const iotSettings = settings.get().alibabacloud_iot;

        if (iotSettings.product_key && iotSettings.device_name && (iotSettings.product_secret || iotSettings.device_secret)) {
            logger.info('Connecting to alibabacloud-iot');

            return new Promise((resolve) => {
                if (iotSettings.device_secret) {
                    const device = iot.device({
                        productKey: iotSettings.product_key,
                        deviceName: iotSettings.device_name,
                        deviceSecret: iotSettings.device_secret
                    });
                    this.client = device.mqttClient;

                    device.on('connect', () => {
                        logger.info('Connected to alibabacloud-iot');
                        resolve();
                    });

                    device.on('message', (topic, message) => {
                        logger.info(`onMessage: topic ${topic}, message ${message}`);
                        this.onMessage(topic, message);
                    });
                } else {
                    iot.register({
                        productKey: iotSettings.product_key,
                        productSecret: iotSettings.product_secret,
                        deviceName: iotSettings.device_name
                    }, (res) => {
                        if (res.code === '200') {
                            const device = iot.device({
                            ...
                            device.on('message', this.onMessage);
                        }
                    });
                }
            });
